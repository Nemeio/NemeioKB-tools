...
set PACKAGENAME="%~dp0/../Nemeio_Keyboard_NRF52_pkg_v11.zip"
set KEYFILE="%~dp0/nemeiopriv.pem"

"%~dp0/../nrfutil" pkg generate --hw-version 52 --sd-req %SDREQ%^
 --softdevice %SDFILE% --sd-id %SDID%^
 --bootloader-version %BLVER% --bootloader %BLHEX% --sd-boot-validation VALIDATE_ECDSA_P256_SHA256 ^
